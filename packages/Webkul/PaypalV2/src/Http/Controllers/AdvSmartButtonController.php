...
use Webkul\PaypalV2\Payment\AdvSmartButton;
use Webkul\Sales\Repositories\InvoiceRepository;
use Webkul\Sales\Repositories\OrderRepository;
use Webkul\Sales\Transformers\OrderResource;

class AdvSmartButtonController extends Controller
{
    /**
    ...
    public function __construct(
        protected AdvSmartButton $advSmartButton,
    ...
    {
        try {
            return response()->json($this->advSmartButton->createOrder($this->buildRequestBody()));
        } catch (\Exception $e) {
    ...
    {
        try {
            $this->advSmartButton->captureOrder(request()->input('orderData.orderID'));
    ...
    protected function buildRequestBody()
    {
        ...
        $data = [
            ...
            'purchase_units' => [
                [
                    'amount'   => [
                        'value'         => $this->advSmartButton->formatCurrencyValue((float) $cart->sub_total + $cart->tax_total + ($cart->selected_shipping_rate ? $cart->selected_shipping_rate->price : 0) - $cart->discount_amount),
                        'currency_code' => $cart->cart_currency_code,

                        'breakdown'     => [
                            'item_total' => [
                                'currency_code' => $cart->cart_currency_code,
                                'value'         => $this->advSmartButton->formatCurrencyValue((float) $cart->sub_total),
                            ],

                            'shipping'   => [
                                'currency_code' => $cart->cart_currency_code,
                                'value'         => $this->advSmartButton->formatCurrencyValue((float) ($cart->selected_shipping_rate ? $cart->selected_shipping_rate->price : 0)),
                            ],

                            'tax_total'  => [
                                'currency_code' => $cart->cart_currency_code,
                                'value'         => $this->advSmartButton->formatCurrencyValue((float) $cart->tax_total),
                            ],

                            'discount'   => [
                                'currency_code' => $cart->cart_currency_code,
                                'value'         => $this->advSmartButton->formatCurrencyValue((float) $cart->discount_amount),
                            ],
                        ],
                    ],
        ...
        if (! empty($cart->billing_address->phone)) {
            $data['payer']['phone'] = [
                ...
                'phone_number' => [
                    'national_number' => $this->advSmartButton->formatPhone($cart->billing_address->phone),
                ],
            ];
        }
    ...
    protected function getLineItems($cart)
    {
        ...
        foreach ($cart->items as $item) {
            $lineItems[] = [
                'unit_amount' => [
                    'currency_code' => $cart->cart_currency_code,
                    'value'         => $this->advSmartButton->formatCurrencyValue((float) $item->price),
                ],
