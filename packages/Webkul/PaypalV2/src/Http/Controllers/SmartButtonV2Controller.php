...
namespace Webkul\PaypalV2\Http\Controllers;

use Webkul\Checkout\Facades\Cart;
use Webkul\PaypalV2\Payment\SmartButtonV2;
use Webkul\Sales\Repositories\InvoiceRepository;
use Webkul\Sales\Repositories\OrderRepository;
use Webkul\Sales\Transformers\OrderResource;

class SmartButtonV2Controller extends Controller
{
    /**
    ...
    public function __construct(
        protected SmartButtonV2 $smartButtonV2,
    ...
    {
        try {
            $d = $this->smartButtonV2->fetchAccessToken();
            // dd($d);
            return $d;
        } catch (\Exception $e) {
    ...
    {
        try {
            return response()->json($this->smartButtonV2->createOrder($this->buildRequestBody()));
        } catch (\Exception $e) {
            return response()->json(json_decode($e->getMessage()), 400);
        }
    }
    ...
    {
        try {
            $this->smartButtonV2->captureOrder(request()->input('orderData.orderID'));
    ...
    protected function buildRequestBody()
    {
        ...
        $data = [
            ...
            'purchase_units' => [
                [
                    'amount'   => [
                        'value'         => $this->smartButtonV2->formatCurrencyValue((float) $cart->sub_total + $cart->tax_total + ($cart->selected_shipping_rate ? $cart->selected_shipping_rate->price : 0) - $cart->discount_amount),
                        'currency_code' => $cart->cart_currency_code,

                        'breakdown'     => [
                            'item_total' => [
                                'currency_code' => $cart->cart_currency_code,
                                'value'         => $this->smartButtonV2->formatCurrencyValue((float) $cart->sub_total),
                            ],

                            'shipping'   => [
                                'currency_code' => $cart->cart_currency_code,
                                'value'         => $this->smartButtonV2->formatCurrencyValue((float) ($cart->selected_shipping_rate ? $cart->selected_shipping_rate->price : 0)),
                            ],

                            'tax_total'  => [
                                'currency_code' => $cart->cart_currency_code,
                                'value'         => $this->smartButtonV2->formatCurrencyValue((float) $cart->tax_total),
                            ],

                            'discount'   => [
                                'currency_code' => $cart->cart_currency_code,
                                'value'         => $this->smartButtonV2->formatCurrencyValue((float) $cart->discount_amount),
                            ],
                        ],
                    ],
        ...
        if (! empty($cart->billing_address->phone)) {
            $data['payer']['phone'] = [
                ...
                'phone_number' => [
                    'national_number' => $this->smartButtonV2->formatPhone($cart->billing_address->phone),
                ],
            ];
        }
    ...
    protected function getLineItems($cart)
    {
        ...
        foreach ($cart->items as $item) {
            $lineItems[] = [
                'unit_amount' => [
                    'currency_code' => $cart->cart_currency_code,
                    'value'         => $this->smartButtonV2->formatCurrencyValue((float) $item->price),
                ],
