...
@pushOnce('scripts')
    ...
    >
        <div class="flex">
            <span
                class="cursor-pointer rounded-md p-1.5 text-2xl transition-all hover:bg-gray-100 dark:hover:bg-gray-950"
                :class="[isDarkModeLocalStorage ? 'icon-light' : 'icon-dark']"
                @click="toggleLocalStorage"
            ></span>
        </div>
    </script>
    ...
    <script type="module">
        app.component('v-dark', {
            ...
            methods: {
                ...
                toggleLocalStorage() {
                    this.isDarkModeLocalStorage = this.getLocalStorage();

                    this.setLocalStorage(this.isDarkModeLocalStorage);

                    document.documentElement.classList.toggle('dark', this.isDarkModeLocalStorage === 1);

                    if (this.isDarkModeLocalStorage) {
                        this.$emitter.emit('change-theme', 'dark');
                ...
                setLocalStorage(switchNumber) {
                    let number = switchNumber === 0 ? 1 : 0;
                    localStorage.setItem("dark_mode", number);

                    this.isDarkModeLocalStorage = number;
                },
