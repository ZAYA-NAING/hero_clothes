...
namespace Webkul\MyanmarPay\Listeners;
...
class Transaction
{
    ...
    public function saveTransaction($invoice)
    {
        ...
        if ($invoice->order->payment->method == 'myanmarpay_manual') {
            if (isset($data['orderData']['orderID'])) {
                $transactionDetails = $this->myanmarPayManual->getOrder($data['orderData']['orderID']);
