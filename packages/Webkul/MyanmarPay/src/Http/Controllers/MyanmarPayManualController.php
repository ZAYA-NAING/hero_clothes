...
namespace Webkul\MyanmarPay\Http\Controllers;

use Webkul\Checkout\Facades\Cart;
use Webkul\MyanmarPay\Payment\MyanmarPayManual;
use Webkul\Sales\Repositories\InvoiceRepository;
use Webkul\Sales\Repositories\OrderRepository;
use Webkul\Sales\Transformers\OrderResource;

class MyanmarPayManualController extends Controller
{
    /**
    ...
    public function __construct(
        protected MyanmarPayManual $myanmarPayManual,
        protected OrderRepository $orderRepository,
        protected InvoiceRepository $invoiceRepository
    ) {}

    /**
     * MyanmarPay order creation for approval of client.
     *
    ...
    protected function buildRequestBody()
    {
        ...
        $data = [
            ...
            'purchase_units' => [
                [
                    'amount'   => [
                        'value'         => $this->myanmarPayManual->formatCurrencyValue((float) $cart->sub_total + $cart->tax_total + ($cart->selected_shipping_rate ? $cart->selected_shipping_rate->price : 0) - $cart->discount_amount),
                        'currency_code' => $cart->cart_currency_code,

                        'breakdown'     => [
                            'item_total' => [
                                'currency_code' => $cart->cart_currency_code,
                                'value'         => $this->myanmarPayManual->formatCurrencyValue((float) $cart->sub_total),
                            ],

                            'shipping'   => [
                                'currency_code' => $cart->cart_currency_code,
                                'value'         => $this->myanmarPayManual->formatCurrencyValue((float) ($cart->selected_shipping_rate ? $cart->selected_shipping_rate->price : 0)),
                            ],

                            'tax_total'  => [
                                'currency_code' => $cart->cart_currency_code,
                                'value'         => $this->myanmarPayManual->formatCurrencyValue((float) $cart->tax_total),
                            ],

                            'discount'   => [
                                'currency_code' => $cart->cart_currency_code,
                                'value'         => $this->myanmarPayManual->formatCurrencyValue((float) $cart->discount_amount),
                            ],
                        ],
                    ],
        ...
        if (! empty($cart->billing_address->phone)) {
            $data['payer']['phone'] = [
                ...
                'phone_number' => [
                    'national_number' => $this->myanmarPayManual->formatPhone($cart->billing_address->phone),
                ],
            ];
        }
    ...
    protected function getLineItems($cart)
    {
        ...
        foreach ($cart->items as $item) {
            $lineItems[] = [
                'unit_amount' => [
                    'currency_code' => $cart->cart_currency_code,
                    'value'         => $this->myanmarPayManual->formatCurrencyValue((float) $item->price),
                ],
